...
| `qualys_asset_tag` | `qps/rest/2.0/.../am/tag` | static + dynamic (`ruleType`/`ruleText`), hierarchy via `parentTagId`; two-call child add/remove | **P0** |
...
| `qualys_was_option_profile` | `.../was/optionprofile` | delete endpoint `Unverified` — confirm before shipping | P2 |
| `qualys_was_auth_record` | `.../was/webappauthrecord` | secrets masked on read → write-only | P2 |
| `qualys_was_scan_schedule` | `.../was/wasscanschedule` | recurrence field model `Unverified` — confirm first | P2–P3 |
...
| `data.qualys_tagged_assets` | `search/am/hostasset` by tag | tag membership review |
...
**Unsupported / legacy (recorded, not planned):** report-schedule create/update/delete
(GUI-only), distribution-group CRUD (no API), network delete (no API), appliance
replace (UI wizard), physical appliance create/delete (no API), scheduled maps
...
WAS report schedules (API `Unverified`).
