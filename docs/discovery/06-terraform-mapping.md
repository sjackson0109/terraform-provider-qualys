...
| `qualys_was_scan_schedule` | `.../was/wasscanschedule` | **Built.** Single-web-app target, type, profile, `scheduling` sub-object (startDate/timeZone.code/occurrenceType/cancelAfterNHours), `DAILY`/`WEEKLY`/`MONTHLY` recurrence (MONTHLY by analogy from the report-schedule walkthrough), full `notification` sub-object, sendMail/sendOneMail/sendMailFromAddressOption, activate/deactivate as dedicated endpoints — Confirmed against a user-supplied Create/Update walkthrough that corrected the top-level flat-field shape a prior pass guessed. **Tag-based multi-app targeting remains unmodelled.** JSON wrapper key `WasScanSchedule` Confirmed (seen verbatim in the walkthrough) | done |
...
| `data.qualys_was_findings` | `.../was/finding` | **Built.** Read-only: search/get. Filter and finding fields from the derived WAS reference (doc 11 §9), Corroborated non-official | done |
