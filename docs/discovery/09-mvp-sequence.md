...
1. VMDR XML client: Basic + session auth, `X-Requested-With`, **secure XML parsing
   (no external DTD/entity resolution)**, `SIMPLE_RETURN` handling, WARNING/`id_min`
   pagination, 409 limit handling with `X-RateLimit-ToWait-Sec`, endpoint-safe retry
   policy (no blind retry of destructive calls).
2. qps client (shared by WAS 3.0 / AM 2.0): ServiceRequest envelope, JSON preference,
   `hasMoreRecords`/`lastId` pagination.
3. Tenant capability probe (doc 08) + resolution of the **design-gating `Unverified`
   items** (asset-group `set_*` names, purge params, CIDR handling, not-found codes).
...
9. `qualys_virtual_scanner` (create → export `activation_code`; VLANs/routes; network
   assignment; optional `wait_for_online`) — with documented handoff to the
   infrastructure provider for VM deployment.
10. `qualys_scan_schedule` (+ data source) — full recurrence/timezone/notification
    model; distribution-group IDs referenced, never raw emails.
